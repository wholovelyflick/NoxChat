...
from keyboards import (
    ...
    # Убираем build_admin_management_keyboard - его больше нет
)
...
async def admin_user_manage(message: Message, bot: Bot):
...
async def admin_all_users(message: Message, bot: Bot):
    ...
    all_users = await db.get_all_users(50)
    
    if not all_users:
        await message.answer("❌ Пользователей не найдено")
        return
    
    user_list = "👥 Последние 50 пользователей:\n\n"
    for i, (tg_id, username, phone, reg_date, blocked, is_admin_user) in enumerate(all_users, 1):
        status = "🚫" if blocked else "✅"
        admin_emoji = "👑" if is_admin_user else ""
        phone_display = phone if phone else "📵"
        
        user_list += f"{i}. {status} {admin_emoji} ID: {tg_id}\n"
        user_list += f"   👤 @{username or 'нет'} | 📱 {phone_display}\n"
...
async def admin_searching(message: Message, bot: Bot):
    ...
    searching = await db.list_searching(30)
...
async def admin_blocked(message: Message, bot: Bot):
    ...
    for i, (tg_id, username, phone, reg_date) in enumerate(blocked_users, 1):
        blocked_text += f"{i}. ID: {tg_id}\n"
        blocked_text += f"   👤 @{username or 'нет'} | 📱 {phone or '📵'}\n"
...
# Убираем раздел "Управление админами" так как клавиатуры больше нет
