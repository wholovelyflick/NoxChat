...
class Database:
    ...
    async def update_profile(self, tg_id: int, phone_number: Optional[str] = None) -> None:

        if phone_number is not None:

            conn = await self.get_connection()

            await conn.execute("UPDATE users SET phone_number = ? WHERE tg_id = ?", (phone_number, tg_id))
    ...
    async def get_profile(self, tg_id: int) -> Dict[str, Optional[object]]:
        ...
        cur = await conn.execute("SELECT phone_number FROM users WHERE tg_id = ?", (tg_id,))
    ...
    async def get_user(self, tg_id: int) -> Optional[Dict[str, Optional[int]]]:
        ...
        cur = await conn.execute("SELECT tg_id, username, phone_number, registered_at, in_search, partner_tg_id, blocked, is_admin FROM users WHERE tg_id = ?", (tg_id,))
        ...
        return {
            ...
            "in_search": int(row[4]) if row[4] is not None else 0,

            "partner_tg_id": int(row[5]) if row[5] is not None else None,

            "blocked": int(row[6]) if row[6] is not None else 0,
    ...
    async def get_all_users(self, limit: int = 100) -> List[tuple]:
        ...
        cur = await conn.execute("SELECT tg_id, username, phone_number, registered_at, blocked, is_admin FROM users ORDER BY registered_at DESC LIMIT ?", (limit,))
    ...
    async def get_blocked_users(self) -> List[tuple]:
        ...
        cur = await conn.execute("SELECT tg_id, username, phone_number, registered_at FROM users WHERE blocked = 1")
